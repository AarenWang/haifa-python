    ...
    def _compile_pipeline(self, stages: List[JQNode], current_reg: str) -> None:
        ...
        if isinstance(stage, FunctionCall):
            ...
            if stage.name == "sort" and len(stage.args) == 0:
                dest = self._new_temp()
                self.instructions.append(Instruction(Opcode.SORT, [dest, current_reg]))
                self._compile_pipeline(rest, dest)
                return
            if stage.name == "sort_by" and len(stage.args) == 1:
                ...
                elem_reg = self._new_temp()
                self.instructions.append(Instruction(Opcode.LOAD_CONST, [index_reg, 0]))
                self.instructions.append(Instruction(Opcode.LEN_VALUE, [length_reg, array_reg]))
                loop_label = self._new_label("jq_sort_by_loop")
                end_label = self._new_label("jq_sort_by_end")
                self.instructions.append(Instruction(Opcode.LABEL, [loop_label]))
                self.instructions.append(Instruction(Opcode.LT, [cond_reg, index_reg, length_reg]))
                self.instructions.append(Instruction(Opcode.JZ, [cond_reg, end_label]))
                self.instructions.append(Instruction(Opcode.GET_INDEX, [elem_reg, array_reg, index_reg]))
                # compute key for element
                key_reg = self._eval_expression(stage.args[0], elem_reg)
                self.instructions.append(Instruction(Opcode.PUSH_EMIT, [keys_buf]))
                self.instructions.append(Instruction(Opcode.EMIT, [key_reg]))
                self.instructions.append(Instruction(Opcode.POP_EMIT, []))
                self.instructions.append(Instruction(Opcode.ADD, [index_reg, index_reg, "1"]))
                self.instructions.append(Instruction(Opcode.JMP, [loop_label]))
                self.instructions.append(Instruction(Opcode.LABEL, [end_label]))
                dest = self._new_temp()
                self.instructions.append(Instruction(Opcode.SORT_BY, [dest, array_reg, keys_buf]))
                self._compile_pipeline(rest, dest)
                return
            if stage.name == "unique" and len(stage.args) == 0:
                dest = self._new_temp()
                self.instructions.append(Instruction(Opcode.UNIQUE, [dest, current_reg]))
                self._compile_pipeline(rest, dest)
                return
            if stage.name == "unique_by" and len(stage.args) == 1:
                ...
                self.instructions.append(Instruction(Opcode.LABEL, [end_label]))
                dest = self._new_temp()
                self.instructions.append(Instruction(Opcode.UNIQUE_BY, [dest, array_reg, keys_buf]))
                self._compile_pipeline(rest, dest)
                return
            if stage.name == "min" and len(stage.args) == 0:
                dest = self._new_temp()
                self.instructions.append(Instruction(Opcode.MIN, [dest, current_reg]))
                self._compile_pipeline(rest, dest)
                return
            if stage.name == "max" and len(stage.args) == 0:
                dest = self._new_temp()
                self.instructions.append(Instruction(Opcode.MAX, [dest, current_reg]))
                self._compile_pipeline(rest, dest)
                return
            if stage.name == "min_by" and len(stage.args) == 1:
                ...
                self.instructions.append(Instruction(Opcode.LABEL, [end_label]))
                dest = self._new_temp()
                self.instructions.append(Instruction(Opcode.MIN_BY, [dest, array_reg, keys_buf]))
                self._compile_pipeline(rest, dest)
                return
            if stage.name == "max_by" and len(stage.args) == 1:
                ...
                self.instructions.append(Instruction(Opcode.LABEL, [end_label]))
                dest = self._new_temp()
                self.instructions.append(Instruction(Opcode.MAX_BY, [dest, array_reg, keys_buf]))
                self._compile_pipeline(rest, dest)
                return
            if stage.name == "group_by" and len(stage.args) == 1:
                ...
                self.instructions.append(Instruction(Opcode.LABEL, [end_label]))
                dest = self._new_temp()
                self.instructions.append(Instruction(Opcode.GROUP_BY, [dest, array_reg, keys_buf]))
                self._compile_pipeline(rest, dest)
                return
