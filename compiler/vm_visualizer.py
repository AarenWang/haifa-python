...
def _font_supports_text(font: "pygame.font.Font", sample: str) -> bool:
    ...
    if not metrics or len(metrics) != len(sample):
        return False

    for metric in metrics:
        # pygame returns ``None`` when a glyph is missing.
        if metric is None:
            return False
        min_x, max_x, _min_y, _max_y, advance = metric
        # Missing glyphs sometimes report a zero-width advance even when
        # ``metrics`` returned a tuple, so we guard against both cases.
        if max_x <= min_x or advance == 0:
            return False
    ...
    # Compare against common tofu glyphs (empty squares or placeholder boxes).
    tofu_variants = [
        "?" * len(sample),
        "\u25A1" * len(sample),  # white square
        "\u3013" * len(sample),  # geta mark
        "\u53E3" * len(sample),  # CJK "口"
    ]

    for fallback_text in tofu_variants:
        try:
            fallback_surface = font.render(fallback_text, True, (0, 0, 0))
        except Exception:
            continue

        if not hasattr(fallback_surface, "get_size"):
            continue

        if fallback_surface.get_size() != (width, height):
            continue

        try:
            if (
                pygame.image.tostring(surface, "RGBA")
                == pygame.image.tostring(fallback_surface, "RGBA")
            ):
                return False
        except Exception:
