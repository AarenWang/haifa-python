    ...
    def _compile_pipeline(self, stages: List[JQNode], current_reg: str) -> None:
        ...
        if isinstance(stage, FunctionCall):
            ...
            if stage.name == "del" and len(stage.args) == 1:
                ...
                self._compile_pipeline(rest, current_reg)
                return
            if stage.name == "map_values" and len(stage.args) == 1:
                keys_reg = self._new_temp()
                idx_reg = self._new_temp()
                len_reg = self._new_temp()
                cond_reg = self._new_temp()
                key_reg = self._new_temp()
                value_reg = self._new_temp()
                mapped_values_reg = self._new_temp()
                result_reg = self._new_temp()
                loop_label = self._new_label("jq_map_values_loop")
                end_label = self._new_label("jq_map_values_end")

                self.instructions.append(Instruction(JQOpcode.KEYS, [keys_reg, current_reg]))
                self.instructions.append(Instruction(Opcode.LOAD_CONST, [idx_reg, 0]))
                self.instructions.append(Instruction(JQOpcode.LEN_VALUE, [len_reg, keys_reg]))
                self.instructions.append(Instruction(Opcode.LOAD_CONST, [mapped_values_reg, []]))
                self.instructions.append(Instruction(Opcode.LABEL, [loop_label]))
                self.instructions.append(Instruction(Opcode.LT, [cond_reg, idx_reg, len_reg]))
                self.instructions.append(Instruction(Opcode.JZ, [cond_reg, end_label]))
                self.instructions.append(Instruction(JQOpcode.GET_INDEX, [key_reg, keys_reg, idx_reg]))
                self.instructions.append(Instruction(JQOpcode.OBJ_GET_DYNAMIC, [value_reg, current_reg, key_reg]))
                self.instructions.append(Instruction(JQOpcode.PUSH_EMIT, [mapped_values_reg]))
                self._compile_pipeline(flatten_pipe(stage.args[0]), value_reg)
                self.instructions.append(Instruction(JQOpcode.POP_EMIT, []))
                self.instructions.append(Instruction(Opcode.ADD, [idx_reg, idx_reg, "1"]))
                self.instructions.append(Instruction(Opcode.JMP, [loop_label]))
                self.instructions.append(Instruction(Opcode.LABEL, [end_label]))
                self.instructions.append(Instruction(JQOpcode.MAP_VALUES, [result_reg, current_reg, mapped_values_reg]))
                self._compile_pipeline(rest, result_reg)
                return
            if stage.name == "with_entries" and len(stage.args) == 1:
                keys_reg = self._new_temp()
                idx_reg = self._new_temp()
                len_reg = self._new_temp()
                cond_reg = self._new_temp()
                key_reg = self._new_temp()
                value_reg = self._new_temp()
                entry_reg = self._new_temp()
                transformed_reg = self._new_temp()
                result_reg = self._new_temp()
                loop_label = self._new_label("jq_with_entries_loop")
                end_label = self._new_label("jq_with_entries_end")

                self.instructions.append(Instruction(JQOpcode.KEYS, [keys_reg, current_reg]))
                self.instructions.append(Instruction(Opcode.LOAD_CONST, [idx_reg, 0]))
                self.instructions.append(Instruction(JQOpcode.LEN_VALUE, [len_reg, keys_reg]))
                self.instructions.append(Instruction(Opcode.LOAD_CONST, [transformed_reg, []]))
                self.instructions.append(Instruction(Opcode.LABEL, [loop_label]))
                self.instructions.append(Instruction(Opcode.LT, [cond_reg, idx_reg, len_reg]))
                self.instructions.append(Instruction(Opcode.JZ, [cond_reg, end_label]))
                self.instructions.append(Instruction(JQOpcode.GET_INDEX, [key_reg, keys_reg, idx_reg]))
                self.instructions.append(Instruction(JQOpcode.OBJ_GET_DYNAMIC, [value_reg, current_reg, key_reg]))
                self.instructions.append(Instruction(Opcode.LOAD_CONST, [entry_reg, {}]))
                self.instructions.append(Instruction(JQOpcode.OBJ_SET, [entry_reg, "key", key_reg]))
                self.instructions.append(Instruction(JQOpcode.OBJ_SET, [entry_reg, "value", value_reg]))
                self.instructions.append(Instruction(JQOpcode.PUSH_EMIT, [transformed_reg]))
                self._compile_pipeline(flatten_pipe(stage.args[0]), entry_reg)
                self.instructions.append(Instruction(JQOpcode.POP_EMIT, []))
                self.instructions.append(Instruction(Opcode.ADD, [idx_reg, idx_reg, "1"]))
                self.instructions.append(Instruction(Opcode.JMP, [loop_label]))
                self.instructions.append(Instruction(Opcode.LABEL, [end_label]))
                self.instructions.append(Instruction(JQOpcode.WITH_ENTRIES, [result_reg, current_reg, transformed_reg]))
                self._compile_pipeline(rest, result_reg)
                return
