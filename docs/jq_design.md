...
2025-09-24（Milestone 6 完成）
- jq 运行时支持字节码缓存与流式输出（`run_filter_stream`），避免为大批量输入构建全量结果列表。
- CLI 新增 `--debug` 参数，失败时可输出完整堆栈；默认情况下返回结构化错误提示。
- README 增补安装/打包指引与 CLI 示例，配套文档《jq_design.md》加入指令分层表。
- 测试套件新增缓存/流式/错误处理等用例，覆盖 CLI debug 与复杂 reduce 错误路径。

### 阶段 3：扩展特性与优化（进行中）
- **更多过滤器**：继续拓展特殊函数与算术在 JSON 值上的语义。
- **性能与内存**：进一步探索迭代器持久化、路径缓存等高级优化策略。
- **错误与诊断**：在现有 `--debug` 的基础上，补充更细粒度的错误分类与位置信息。
- **文档与示例**：沉淀更系统的用户指南、脚本样例与性能调优说明。
- **测试计划**：引入更大规模数据集与压力测试脚本。
