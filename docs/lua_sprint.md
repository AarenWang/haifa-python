...
### Milestone 5：表与表达式扩展
- [ ] AST/Parser：支持表构造器（数组部分、键值对）、字段访问/索引表达式、方法调用语法。
- [ ] 赋值语义：允许多目标赋值、`local` 批量声明，并与多返回值/vararg 对齐。
...
- [ ] 语法支持：补齐 `elseif`、`repeat ... until`、`break`、`do ... end` 块。
- [ ] 循环结构：实现数值 `for`（含步长）、泛型 `for`（配合迭代器）以及循环局部变量作用域。
