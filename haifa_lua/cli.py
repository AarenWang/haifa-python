...
def _stack_summary(stack) -> str:
    if not stack:
        return "∅"
    frame = stack[0]
    return f"{frame.function_name}@{frame.file}:{frame.line}"
...
def _format_event(event: object) -> str:
    if isinstance(event, (CoroutineCreated,)):
        ...
        stack = _stack_summary(event.stack)
        extra = ""
        if event.upvalues:
    ...
    if isinstance(event, CoroutineResumed):
        scheduler_stack = _stack_summary(event.stack)
        target = f" next_pc={event.coroutine_pc}" if event.coroutine_pc is not None else ""
        target_stack = (
            f" target={_stack_summary(event.coroutine_stack)}"
    ...
    if isinstance(event, CoroutineCompleted):
        if event.error:
            return (
                ...
                f"pc={event.pc} stack={_stack_summary(event.stack)}"
            )
        return (
